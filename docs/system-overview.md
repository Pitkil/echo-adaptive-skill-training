...
| 产品身份 | 主要界面 | 能够完成的工作 |
|---|---|---|
| 学习者 | 课程中心、ECHO 导学工作台、视频伴学、学习画像、个性化资源、语音证据 | 选择已开放课程、对话学习、观看课程视频、答题、查看报告、提交授权语音 |
...
视频伴学使用浏览器原生播放器。讲师或系统管理员在“内容导入”中按模块批量上传课程视频，
文件保存在服务器的运行数据目录（不进入 Git），并在业务库中登记元数据。学习者看到当前
模块的视频列表，播放进度按用户和视频保存到服务器，可断点续看。视频播放、暂停和拖动
进度不会自动开启麦克风；
系统只在学习节点暂停并给出口述练习提示。学习节点口述题由讲师触发“抽帧 + OCR”自动生成草稿，
经人工核对评分要点和 Microsoft 官方出处并冻结后固定使用，同时绑定稳定的 `practice` 评分记录。
学习者进入“语音证据”页、明确勾选本轮授权并主动开始录音后，音频才会并行提交 ASR 与
微表征服务。ASR 转写必须由学习者确认或纠错；确认文本由 AI 按冻结要点做语义匹配，服务端计算
覆盖分数并通过唯一 `attempt_id` 更新 MIRT。微表征只调整提示节奏与辅导方式，不参与对错和
U/A/R 计算；普通语音证据不绑定检查点时也不会产生能力分数。
