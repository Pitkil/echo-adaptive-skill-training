...
> 本文保留 2026 年 8 月 26 日改版前的根目录 README 内容，作为详细开发与协作参考。
> 面向首次访问者的产品说明、快速启动和当前状态请返回 [项目首页](../README.md)。
...
| 需要了解的问题 | 阅读文档 |
|---|---|
| 系统要做什么、有哪些功能、完整流程是什么 | [系统功能与架构说明](system-overview.md) |
| A、B、C、D 分别做什么、先后顺序是什么 | [四人任务与时间线](team-ownership.md) |
| 数据怎样在主系统、检索引擎、SimpleMem 和微表征之间传递 | [服务接口契约](service-contracts.md) |
| 分支、Commit、PR、命名和合并要求 | [协作与开发规范](collaboration.md) |
| 改完后需要通过哪些测试 | [测试与质量门禁](testing-and-quality.md) |
| 功能是否满足赛题、最终看哪些指标 | [赛题要求映射](competition-requirements.md) |

固定题库的准备格式见 [题库导入模板](quiz-import-template.md)。完整文档导航见
...
2. [四人任务与时间线](team-ownership.md)：确认自己的任务、交付和配合对象。
3. [协作与开发规范](collaboration.md)：确认分支、Commit、PR 和合并要求。
...
完整流程、架构和功能说明见 [系统功能与架构说明](system-overview.md)，
其他工程文档见 [docs/README.md](README.md)。
