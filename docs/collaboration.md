...
- 仓库保持 Private，由负责人邀请 A、B、C、D 成为 Collaborator。
- 固定成员直接 Clone 同一个仓库，不使用 Fork。
- 四个人各使用一个固定分支：
...
GitHub 套餐支持时，为 `main` 开启：必须通过 PR、必须通过自动检查、必须解决讨论、
禁止强制推送和禁止删除。Private 仓库无法启用相关设置时，上述限制仍作为团队规则执行。
