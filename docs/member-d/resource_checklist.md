...
| ❌ 不可执行 | 步骤模糊、命令错误、无法复现 | 仅退回并重做失败步骤 |
...
| ⚠️ 基本适配 | 内容与学习者水平大致匹配 | 标注后通过 |
| ❌ 不适配 | 内容与学习者水平明显不匹配 | 仅重做不适配部分，并明确难度、深度或步骤问题 |

**判断依据**（来自 `PersonalizationPlan`）：
- `difficulty`：难度等级（foundation/standard/advanced）
- `weakest_dimension_label`：重点补强维度（理解/应用/推理）
- `support_strategy`：支持策略（拆分步骤/提供提示）
- `reason`：完整的个性化原因（包含能力分析、盲区、记忆提示等）

**不适配示例**：
- P1 学习者收到 advanced 难度的内容，解释深度过深
- P3 学习者收到 foundation 难度的内容，过于简单
- P1 学习者收到缺少详细步骤的操作指南
- 内容未针对 `weakest_dimension_label` 进行补强（如"推理"最弱但只有理解题）
