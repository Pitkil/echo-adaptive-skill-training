...
以下 15 条 Microsoft 官方来源与 `official_materials_manifest.json` 一致。当前完成的是来源登记；仓库中没有保存官方原始文件，因此不能据此声称材料文件已经导入或通过检索验证。
...
connection refused。因此以下“已导入、可检索、可引用”仍保持未完成，未伪造 `kb_id`、
`document_id`、`task_id` 或 `indexed` 状态。
